...
HIW_loglik = function(u, params) {
  ...
  logprior = - 0.5 * N * D * log(2 * pi) + N * hybrid::log_det(Lt) -
...
HIWsim = function(Adj, bG, DG){
  ...
  Sigmaj[c1, c1] = solve(Wishart_InvA_RNG(
    bG+length(Cliques[[1]])-1, DG[Cliques[[1]], Cliques[[1]]] ))

  for(i in 2:numberofcliques){ # visit components and separators in turn
    dif = sort(setdiff(Cliques[[i]], UN))
    UN = sort(union(Cliques[[i]], UN)) # probably no need to sort
    sizedif = length(dif)
    sid = Separators[[i]]
    if(length(sid)==0){
      ...
      Sigmaj[dif, dif] = SigRS +
        Sigmaj[dif, sid] %*% solve(Sigmaj[sid, sid]) %*% Sigmaj[sid, dif]
    }
  }
  ...
  for(i in 2:numberofcliques){
    ...
    if(length(sid)==0){
      ...
      H = sort(union(H, Cliques[[i]])) # probably no need to sort
    }
    else{
      h = sort(setdiff(H, sid))
      Sigmaj[dif, h] = Sigmaj[dif, sid] %*%
        solve(Sigmaj[sid, sid]) %*% Sigmaj[sid, h]
      Sigmaj[h, dif] = t(Sigmaj[dif, h])
      H = sort(union(H, Cliques[[i]])) # probably no need to sort
    }
  }
