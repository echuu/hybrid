...
double ep(arma::vec m, arma::mat K, arma::vec lb, arma::vec ub) {
	...
	while (!converged) {
		...
		// Rcpp::Rcout << "tau_cavity: \n" << tau_cavity << std::endl;
		// Rcpp::Rcout << "nu_cavity: \n" << nu_cavity << std::endl;
		...
		if (arma::any(tau_site < 0)) {
			Rcpp::Rcout << "negative value for tau" << std::endl;
			// Rcpp::Rcout << "tau_site: " << tau_site << std::endl;
			for (int i = 0; i < tau_site.n_elem; i++) {
				if ((tau_site(i) < 0) && (tau_site(i) > -1e-8)) {
					tau_site(i) = 0.0;
				}
			}
		}
		...
		// Rcpp::Rcout << "improvement: " << (arma::norm(mu_last - mu)) << std::endl;
